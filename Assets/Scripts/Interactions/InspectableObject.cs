...
/// <summary>Which inspect UI layout InspectableUIController opens for this object.</summary>
public enum InspectContentKind
{
    /// <summary>3D studio preview + rotate (default).</summary>
    Model3D = 0,
    /// <summary>Scrollable text page; no 3D clone / no E rotate.</summary>
    TextPage = 1
}
...
public sealed class InspectableObject : MonoBehaviour
{
    ...
    [SerializeField, Tooltip("Model3D = studio preview. TextPage = scrollable text body (no 3D / no rotate).")]
    private InspectContentKind contentKind = InspectContentKind.Model3D;
    ...
    [SerializeField, TextArea(2, 5), Tooltip("One-sentence description shown to the right of the image / text page.")] private string description = "A detail that may be worth remembering.";
    [SerializeField, TextArea(8, 24), Tooltip("Full page body used when Content Kind = Text Page.")]
    private string textPageBody = "";
    ...
    public InspectContentKind ContentKind => contentKind;
    ...
    public string TextPageBody => textPageBody;
    ...
    public void SetContentKind(InspectContentKind kind) => contentKind = kind;

    public void SetTextPage(string title, string body)
    {
        contentKind = InspectContentKind.TextPage;
        if (!string.IsNullOrWhiteSpace(title))
            description = title;
        textPageBody = body ?? string.Empty;
    }
