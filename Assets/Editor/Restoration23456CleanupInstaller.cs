...
{
    [InitializeOnLoadMethod]private static void CleanupAfterCompile(){EditorApplication.delayCall+=()=>{if(!Application.isPlaying&&!EditorApplication.isPlayingOrWillChangePlaymode)Cleanup();};}
    [MenuItem("Tools/Restoration/Cleanup And Keep Items 2-6 Only")]
    public static void Cleanup()
    {
        if(Application.isPlaying)return;
        DeleteByName("PlacedFrame");DeleteByName("FramePlacePoint");DeleteByName("Photoes");DeleteByName("CollectedFamilyPhotoStatus");DeleteByName("PhotoPickupInventory");
